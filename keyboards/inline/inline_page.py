...
def category_open_edit_ap(remover):
    ...
    if len(get_categories) <= 10:
        ...
        next_kb = InlineKeyboardButton(
            "➡ Далее ➡", callback_data=f"edit_catategory_nextp:{remover + count_page}"
        )
        keyboard.add(nomer_kb, next_kb)
    elif remover + count_page >= len(get_categories):
        prev_kb = InlineKeyboardButton(
            "⬅ Назад ⬅", callback_data=f"edit_catategory_prevp:{remover - count_page}"
        )
        nomer_kb = InlineKeyboardButton(
            f"🔸 {str(remover + count_page)[:-1]} 🔸", callback_data="..."
        )
...
def position_open_create_ap(remover):
    ...
    if len(get_categories) <= 10:
        ...
        nomer_kb = InlineKeyboardButton("🔸 1 🔸", callback_data="...")
        next_kb = InlineKeyboardButton(
            "➡ Далее ➡", callback_data=f"create_position_nextp:{remover + count_page}"
        )
        keyboard.add(nomer_kb, next_kb)
    return keyboard
...
def position_create_next_page_ap(remover):
    ...
    if remover + count_page >= len(get_categories):
        prev_kb = InlineKeyboardButton(
            "⬅ Назад ⬅", callback_data=f"create_position_prevp:{remover - count_page}"
        )
        nomer_kb = InlineKeyboardButton(
            f"🔸 {str(remover + count_page)[:-1]} 🔸", callback_data="..."
        )
...
def position_open_edit_ap(remover, category_id):
    ...
    for a in range(remover, len(get_positions)):
        if x < count_page:
            ...
            keyboard.add(
                InlineKeyboardButton(
                    f"{get_positions[a][2]} | {get_positions[a][3]}руб | {len(get_items)}шт",
    ...
    if len(get_positions) <= 10:
        ...
        keyboard.add(prev_kb, nomer_kb, next_kb)
    keyboard.add(
        InlineKeyboardButton("🔄 В главное меню", callback_data=f"back_to_category")
    )
    return keyboard
...
def item_open_add_category_ap(remover):
    ...
    if len(get_categories) <= 10:
        ...
        nomer_kb = InlineKeyboardButton("🔸 1 🔸", callback_data="...")
        next_kb = InlineKeyboardButton(
            "➡ Далее ➡", callback_data=f"add_item_category_nextp:{remover + count_page}"
        )
        keyboard.add(nomer_kb, next_kb)
    elif remover + count_page >= len(get_categories):
        prev_kb = InlineKeyboardButton(
            "⬅ Назад ⬅", callback_data=f"add_item_category_prevp:{remover - count_page}"
        )
        nomer_kb = InlineKeyboardButton(
            f"🔸 {str(remover + count_page)[:-1]} 🔸", callback_data="..."
        )
...
def position_add_item_position_ap(remover, category_id):
    ...
    keyboard.add(
        InlineKeyboardButton(
            "🔄 В главное меню", callback_data=f"back_add_item_to_category"
        )
    )
    return keyboard
...
def buy_item_open_category_ap(remover):
    ...
    keyboard = InlineKeyboardMarkup()
    get_categories = get_all_categoriesx()
    # menu_shop = InlineKeyboardButton("🔍 Меню", callback_data="menu_shop")
    # keyboard.row(menu_shop)
    for a in range(remover, len(get_categories)):
        if x < count_page:
            print(get_categories[a][2])
            keyboard.add(
                InlineKeyboardButton(
                    f"{get_categories[a][2]}",
    ...
    if len(get_categories) <= 10:
        ...
        nomer_kb = InlineKeyboardButton("🔸 1 🔸", callback_data="...")
        next_kb = InlineKeyboardButton(
            "➡ Далее ➡", callback_data=f"buy_category_nextp:{remover + count_page}"
        )
        keyboard.add(nomer_kb, next_kb)
    elif remover + count_page >= len(get_categories):
        prev_kb = InlineKeyboardButton(
            "⬅ Назад ⬅", callback_data=f"buy_category_prevp:{remover - count_page}"
        )
        nomer_kb = InlineKeyboardButton(
            f"🔸 {str(remover + count_page)[:-1]} 🔸", callback_data="..."
        )
        keyboard.add(prev_kb, nomer_kb, menu_shop)
    else:
        next_kb = InlineKeyboardButton(
            "➡ Далее ➡", callback_data=f"buy_category_nextp:{remover + count_page}"
        )
        ...
        keyboard.add(prev_kb, nomer_kb, menu_shop, next_kb)
    return keyboard
...
def buy_item_item_position_ap(remover, category_id):
    ...
    keyboard.add(
        InlineKeyboardButton(
            "🔄 В главное меню", callback_data=f"back_buy_item_to_category"
        )
    )
    return keyboard
