...
from keyboards.default import (
    items_default,
    skip_send_image_default,
    cancel_send_image_default,
...
async def category_open_edit(message: types.Message, state: FSMContext):
    ...
    if len(get_categories) >= 1:
        ...
        await message.answer(
            "<b>📜 Выберите категорию для изменения 🖍</b>", reply_markup=get_kb
        )
    else:
...
async def position_create_new(message: types.Message, state: FSMContext):
    ...
    if len(get_categories) >= 1:
        ...
        await message.answer(
            "<b>📁 Выберите место для позиции ➕</b>", reply_markup=get_kb
        )
    else:
...
async def choice_category_for_edit_position(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "<b>📁 Выберите категорию с нужной вам позицией 🖍</b>", reply_markup=get_kb
    )
...
async def open_edit_items(message: types.Message, state: FSMContext):
    await state.finish()
    await message.answer(
        "<b>🔸 Получение всех товаров и их позиций:</b> \n/getinfoitems\n"
        "<b>🔸 Получение всех позиций:</b> \n/getposition\n"
        "<b>🔸 Получение всех товаров:</b> \n/getitems\n"
        "<b>🔸 Получение базы данных:</b> \n/getbd\n",
...
async def category_create_input_name(message: types.Message, state: FSMContext):
    ...
    await state.finish()
    await message.answer(
        "<b>📜 Категория была успешно создана ✅</b>", reply_markup=items_default
    )
...
@dp.callback_query_handler(
    IsAdmin(), text_startswith="edit_catategory_nextp", state="*"
)
...
@dp.callback_query_handler(
    IsAdmin(), text_startswith="edit_catategory_prevp", state="*"
)
...
async def category_open_for_edit(call: CallbackQuery, state: FSMContext):
    ...
    await bot.edit_message_text(
        messages, call.from_user.id, call.message.message_id, reply_markup=keyboard
    )
...
async def category_edit_name(call: CallbackQuery, state: FSMContext):
    ...
    await bot.send_message(
        call.from_user.id, "<b>🏷 Введите новое название для категории:</b>"
    )
...
async def category_name_was_changed(message: types.Message, state: FSMContext):
    ...
    await state.finish()
    await message.answer(
        "<b>📜 Название было успешно изменено ✅</b>", reply_markup=items_default
    )
    messages, keyboard = edit_category_func(category_id, remover)
...
async def category_remove_confirm(call: CallbackQuery, state: FSMContext):
    ...
    await bot.edit_message_text(
        "<b>📜 Категория и все её данные были успешно удалены ✅</b>",
        call.from_user.id,
        call.message.message_id,
    )
    get_kb = category_open_edit_ap(remover)
...
@dp.callback_query_handler(
    IsAdmin(), text_startswith="confirm_clear_category", state="*"
)
async def category_remove_all_confirm(call: CallbackQuery, state: FSMContext):
    ...
    clear_itemx()
    await bot.edit_message_text(
        "<b>☑ Вы успешно удалили все категории, позиции и товары</b>",
        call.from_user.id,
        call.message.message_id,
    )
...
@dp.callback_query_handler(
    IsAdmin(), text_startswith="cancel_clear_category", state="*"
)
async def category_remove_all_cancel(call: CallbackQuery, state: FSMContext):
    await bot.edit_message_text(
        "<b>☑ Вы отменили удаление всех категорий ☑</b>",
        call.from_user.id,
        call.message.message_id,
    )
...
@dp.callback_query_handler(
    IsAdmin(), text_startswith="create_position_nextp", state="*"
)
async def position_next_page(call: CallbackQuery, state: FSMContext):
    ...
    await bot.edit_message_text(
        "<b>📁 Выберите место для позиции ➕</b>",
        call.from_user.id,
        call.message.message_id,
        reply_markup=get_kb,
    )
...
@dp.callback_query_handler(
    IsAdmin(), text_startswith="create_position_prevp", state="*"
)
...
async def position_select_category_for_create(call: CallbackQuery, state: FSMContext):
    ...
    await bot.delete_message(call.from_user.id, call.message.message_id)
    await bot.send_message(call.from_user.id, "<b>📁 Введите название для позиции 🏷</b>")
...
async def position_input_price(message: types.Message, state: FSMContext):
    if message.text.isdigit():
        if int(message.text) >= 0:
            ...
            await message.answer(
                "В описании надо ОБЯЗАТЕЛЬНО указать название фотографии с папки images в таких скобках []\n пример: [burger.jpg]"
            )
        else:
            await StoragePosition.here_input_position_price.set()
            await message.answer(
                "<b>❌ Минимальная цена для позиции 0 рублей.</b>\n"
...
async def position_input_discription(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer(
            ...
            reply_markup=skip_send_image_default,
        )
...
@dp.message_handler(
    IsAdmin(), text="📸 Пропустить", state=StoragePosition.here_input_position_image
)
async def position_skip_get_image(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "<b>📁 Позиция была успешно создана ✅</b>", reply_markup=items_default
    )


# Принятие изображения позиции для её создания
@dp.message_handler(
    content_types=["photo"], state=StoragePosition.here_input_position_image
)
...
async def back_to_all_categories_for_edit_position(
    call: CallbackQuery, state: FSMContext
):
    get_kb = position_open_edit_category_ap(0)

    await bot.edit_message_text(
        "<b>📁 Выберите категорию с нужной вам позицией 🖍</b>",
        call.from_user.id,
        call.message.message_id,
        reply_markup=get_kb,
    )
...
@dp.callback_query_handler(
    IsAdmin(), text_startswith="edit_position_category_nextp", state="*"
)
...
@dp.callback_query_handler(
    IsAdmin(), text_startswith="edit_position_category_prevp", state="*"
)
async def previous_page_category_for_edit_position(
    call: CallbackQuery, state: FSMContext
):
    remover = int(call.data.split(":")[1])
...
@dp.callback_query_handler(
    IsAdmin(), text_startswith="position_edit_category", state="*"
)
async def open_category_for_edit_position(call: CallbackQuery, state: FSMContext):
    ...
    if len(get_positions) >= 1:
        ...
        await bot.answer_callback_query(
            call.id, "📁 Позиции в данной категории отсутствуют"
        )
...
async def open_for_edit_position(call: CallbackQuery, state: FSMContext):
    ...
    messages, keyboard, have_photo = open_edit_position_func(
    ...
    if have_photo:
        ...
        await bot.send_photo(
            call.from_user.id, get_position[5], messages, reply_markup=keyboard
        )
    else:
        await bot.edit_message_text(
            messages, call.from_user.id, call.message.message_id, reply_markup=keyboard
...
async def back_to_all_categories_for_choice_edit(
    call: CallbackQuery, state: FSMContext
):
    category_id = int(call.data.split(":")[1])
    remover = int(call.data.split(":")[2])
    ...
    if len(get_positions) >= 1:
        ...
        await bot.edit_message_text(
            "<b>❗ Позиции в данной категории отсутствуют</b>",
            call.from_user.id,
            call.message.message_id,
        )
...
async def change_position_name(call: CallbackQuery, state: FSMContext):
    ...
    await bot.delete_message(call.from_user.id, call.message.message_id)
    await bot.send_message(
        call.from_user.id, "<b>🏷 Введите новое название для позиции</b>"
    )
...
async def input_new_position_name(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "<b>✅ Название позиции было успешно изменено</b>", reply_markup=items_default
    )
    await state.finish()
...
@dp.callback_query_handler(
    IsAdmin(), text_startswith="position_change_price", state="*"
)
async def change_position_price(call: CallbackQuery, state: FSMContext):
    ...
    await bot.delete_message(call.from_user.id, call.message.message_id)
    await bot.send_message(call.from_user.id, "<b>💰 Введите новую цену для позиции</b>")
...
async def input_new_position_price(message: types.Message, state: FSMContext):
    if message.text.isdigit():
        if int(message.text) >= 0:
            ...
            await message.answer(
                "<b>✅ Цена позиции была успешно изменена</b>",
            ...
            if have_photo:
                await message.answer_photo(
                    get_position[5], messages, reply_markup=keyboard
                )
            else:
                await message.answer(messages, reply_markup=keyboard)
        else:
            await StoragePosition.here_change_position_name.set()
            await message.answer(
                "<b>❌ Цена не может быть меньше 0.</b>\n" "📁 Введите цену для позиции 💰"
            )
    else:
        await StoragePosition.here_change_position_name.set()
        await message.answer(
...
@dp.callback_query_handler(
    IsAdmin(), text_startswith="position_change_discription", state="*"
)
async def change_position_discription(call: CallbackQuery, state: FSMContext):
    ...
    await bot.delete_message(call.from_user.id, call.message.message_id)
    await bot.send_message(
        call.from_user.id, "<b>📜 Введите новое описание для позиции</b>"
    )
...
async def input_position_discription(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            "<b>✅ Описание позиции было успешно изменено</b>",
            reply_markup=items_default,
        )
        await state.finish()
...
@dp.callback_query_handler(
    IsAdmin(), text_startswith="position_change_photo", state="*"
)
...
@dp.message_handler(
    IsAdmin(), text="📸 Отменить", state=StoragePosition.here_change_position_image
)
async def cancel_input_position_photo(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "<b>✅ Изображение позиции было успешно изменено</b>", reply_markup=items_default
    )
    await state.finish()
...
@dp.message_handler(
    content_types=["photo"], state=StoragePosition.here_change_position_image
)
...
async def open_category_for_create_position(call: CallbackQuery, state: FSMContext):
    ...
    await bot.edit_message_text(
        "<b>☑ Вы успешно удалили позицию и её товары</b>",
        call.from_user.id,
        call.message.message_id,
    )
...
@dp.callback_query_handler(
    IsAdmin(), text_startswith="confirm_clear_position", state="*"
)
async def create_input_position_name(call: CallbackQuery, state: FSMContext):
    await bot.delete_message(call.from_user.id, call.message.message_id)
    delete_msg = await bot.send_message(
        call.from_user.id, "<b>⌛ Ждите, позиции удаляются...</b>"
    )
    ...
    clear_itemx()
    await bot.edit_message_text(
        f"<b>☑ Вы успешно удалили все позиции({get_positions}шт) и товары({get_items}шт)</b>",
        call.from_user.id,
...
@dp.callback_query_handler(
    IsAdmin(), text_startswith="cancel_clear_position", state="*"
)
async def create_input_position_name(call: CallbackQuery, state: FSMContext):
    await bot.edit_message_text(
        "<b>☑ Вы отменили удаление всех позиций</b>",
        call.from_user.id,
        call.message.message_id,
    )
...
@dp.callback_query_handler(
    IsAdmin(), text_startswith="back_add_item_to_category", state="*"
)
...
@dp.callback_query_handler(
    IsAdmin(), text_startswith="add_item_category_nextp", state="*"
)
async def next_page_category_for_edit_position(call: CallbackQuery, state: FSMContext):
    remover = int(call.data.split(":")[1])
...
@dp.callback_query_handler(
    IsAdmin(), text_startswith="add_item_category_prevp", state="*"
)
async def previous_page_category_for_edit_position(
    call: CallbackQuery, state: FSMContext
):
...
@dp.callback_query_handler(
    IsAdmin(), text_startswith="add_item_position_nextp", state="*"
)
async def next_page_for_edit_position(call: CallbackQuery, state: FSMContext):
    remover = int(call.data.split(":")[1])
    category_id = int(call.data.split(":")[2])
...
@dp.callback_query_handler(
    IsAdmin(), text_startswith="add_item_position_prevp", state="*"
)
async def previous_page_for_edit_position(call: CallbackQuery, state: FSMContext):
    remover = int(call.data.split(":")[1])
    category_id = int(call.data.split(":")[2])
...
async def input_item_data(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"{category_id} | {position_id} | {get_all_items}")
    await message.answer(
        f"<b>📥 Товары в кол-ве</b> <u>{count_add}шт</u> <b>были успешно добавлены ✅</b>"
    )


# 📢 Добавить 2 свободных мест для бронирования


@dp.message_handler(IsAdmin(), text="📢 Добавить 2 свободных мест для бронирования")
async def finish_load_items2zxc(message: types.Message):
    count_add = 0
    get_all_items = [f"⚡ | Бронирование места в кафе.\n{current_datetime}"]

    category_id = "125609137"
    position_id = "510658553"
...
@dp.message_handler(IsAdmin(), text="🎫 Карусель")
async def finish_load_items2zxc(message: types.Message):
    count_add = 0
    get_all_items = [f'⚡ | Билет на аттракцион <b>"Карусель"</b>\n{current_datetime}']

    category_id = "382043304"
    position_id = "865769734"
...
@dp.message_handler(IsAdmin(), text="🎫 Колесо обозрения")
async def finish_load_items2zxc(message: types.Message):
    count_add = 0
    get_all_items = [
        f'⚡ | Билет на аттракцион <b>"колесо обозрения"</b>\n{current_datetime}'
    ]

    category_id = "382043304"
    position_id = "692734953"
    for i in range(10):
        add_itemx(
            category_id,
            position_id,
...
@dp.message_handler(IsAdmin(), text="🎫 Комната смеха")
async def finish_load_items2zxc(message: types.Message):
    count_add = 0
    get_all_items = [
        f'⚡ | Билет на аттракцион <b>"Комната смеха"</b>\n{current_datetime}'
    ]

    category_id = "382043304"
    position_id = "536244465"
    for i in range(10):
        add_itemx(
            category_id,
            position_id,
...
async def input_item_id_for_delete(message: types.Message, state: FSMContext):
    ...
    await state.finish()
    await message.answer(
        f"<b>✅ Успешно удалённые товары:</b>\n"
...
async def confirm_clear_all_items(call: CallbackQuery, state: FSMContext):
    await bot.delete_message(call.from_user.id, call.message.message_id)
    delete_msg = await bot.send_message(
        call.from_user.id, "<b>⌛ Ждите, товары удаляются...</b>"
    )
    get_items = get_all_itemsx()

    clear_itemx()
    await bot.edit_message_text(
        "<b>☑ Вы успешно удалили все товары</b>",
        call.from_user.id,
        delete_msg.message_id,
    )
...
async def cancel_remove_all_items(call: CallbackQuery, state: FSMContext):
    await bot.edit_message_text(
        "<b>☑ Вы отменили удаление всех товаров</b>",
        call.from_user.id,
        call.message.message_id,
    )
