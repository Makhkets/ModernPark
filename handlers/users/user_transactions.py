...
number_qiwi = config["settings"]["number_qiwi"]


###################################################################################
############################## ВВОД СУММЫ ПОПОЛНЕНИЯ ##############################
# Выбор способа пополнения
@dp.callback_query_handler(text="user_input1", state="*")
async def input_amount(call: CallbackQuery, state: FSMContext):
    check_pass = False
    # get_payment = get_paymentx()

    number_qiwi = config["settings"]["number_qiwi"]
    token_qiwi = config["settings"]["token_qiwi"]
    secret_key_qiwi = config["settings"]["secret_key_qiwi"].replace("'", "")
    if number_qiwi != "None" and token_qiwi != "None" and secret_key_qiwi != "None":
        try:
            request = requests.Session()
            request.headers["authorization"] = "Bearer " + token_qiwi
            response_qiwi = request.get(
                f"https://edge.qiwi.com/payment-history/v2/persons/{number_qiwi}/payments",
...
async def input_amount(call: CallbackQuery, state: FSMContext):
    ...
    if get_payment[5] == "True":
        if (
            ...
            if check_pass:
                await bot.answer_callback_query(
                    call.id, "❗ Пополнение временно недоступно"
                )
                await send_all_admin(
                    f"👤 Пользователь <a href='tg://user?id={call.from_user.id}'>{clear_firstname(call.from_user.first_name)}</a> "
                    f"пытался пополнить баланс\n"
        ...
        await bot.answer_callback_query(
            call.id, "❗ Пополнения в боте временно отключены"
        )
...
async def create_qiwi_pay(message: types.Message, state: FSMContext):
    if message.text.isdigit():
        ...
        del_msg = await bot.send_message(
            message.from_user.id, "<b>♻ Подождите, платёж генерируется...</b>"
        )
            ...
            try:
                ...
                if pay_amount >= min_input_qiwi:
                    ...
                    if get_payments[4] == "form":
                        ...
                        way_pay = "Number"
                        send_requests = (
                            f"https://qiwi.com/payment/form/99?extra%5B%27account%27%5D={get_payments[0]}&amountInteger="
                        ...
                        way_pay = "Nickname"
                        send_requests = (
                            f"https://qiwi.com/payment/form/99999?amountInteger={pay_amount}&amountFraction=0&currency=643"
                    ...
                    await message.answer(
                        send_message,
                        reply_markup=create_pay_qiwi_func(
                            send_requests,
    ...
    if (
        ...
        if pay_status == "PAID":
            ...
            if get_purchase is None:
                ...
                await bot.answer_callback_query(
                    call.id, "❗ Ваше пополнение уже зачислено", True
                )
            ...
            await bot.answer_callback_query(
                call.id, "❗ Оплата не была произведена", True
            )
        ...
        await send_all_admin(
            "<b>❗ Кто-то пытался проверить платёж, но QIWI не работает\n"
            ...
            for check_pay in get_history:
                if str(receipt) == str(check_pay["comment"]):
                    if "643" == str(check_pay["sum"]["currency"]):
                        ...
                        await bot.answer_callback_query(
                            call.id, "❗ Оплата была произведена не в рублях", True
                        )
            if pay_status:
                get_purchase = get_refillx("*", receipt=receipt)
                if get_purchase is None:
