...
async def search_profile(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "<b>📱 Введите логин или айди пользователя. Пример:</b>\n"
        "▶ 123456789\n"
        "▶ @example"
    )
...
@dp.message_handler(text="📃 Поиск чеков 🔍", state="*")
async def search_receipt(message: types.Message, state: FSMContext):
    await state.finish()
    await message.answer(
        "<b>📃 Отправьте номер чека. Пример:</b>\n" "▶ +123456789\n" "▶ #F123456789"
    )
...
@dp.callback_query_handler(
    IsAdmin(), text=["not_send_kb", "yes_send_ad"], state=StorageFunctions.here_ad_text
)
...
async def input_data_for_search_profile(message: types.Message, state: FSMContext):
    ...
    if get_user_id is not None:
        await message.answer(
            search_user_profile(get_user_id[1]),
...
async def change_user_sale(call: CallbackQuery, state: FSMContext):
    ...
    if len(last_purchases) >= 1:
        ...
        await call.message.answer(
            search_user_profile(user_id), reply_markup=search_profile_func(user_id)
        )
    else:
...
async def input_add_balance(message: types.Message, state: FSMContext):
    if message.text.isdigit():
        ...
        if get_amount >= 1:
            ...
            logger.debug(user_id, balance=int(get_user[4]) + get_amount)
            await message.answer(
                "<b>✅ Пользователю</b> "
            ...
            await bot.send_message(
                user_id, f"<b>💳 Вам было выдано</b> <code>{get_amount}руб</code>"
            )
            await message.answer(
                search_user_profile(user_id), reply_markup=search_profile_func(user_id)
            )
            ...
            await message.answer(
                "<b>❌ Минимальная сумма выдачи 1руб</b>\n"
...
async def input_set_balance(message: types.Message, state: FSMContext):
    if message.text.isdigit():
        ...
        if get_amount >= 0:
            ...
            await message.answer(
                "<b>❌ Минимальная сумма баланса 0руб</b>\n"
...
async def input_search_receipt(message: types.Message, state: FSMContext):
    ...
    if message.text.startswith("+"):
        ...
        if get_input is not None:
            ...
            await message.answer(
                f"<b>📃 Чек:</b> <code>+{get_input[6]}</code>\n"
                "➖➖➖➖➖➖➖➖➖➖➖➖➖\n"
        ...
        if get_purchase is not None:
            ...
            await message.answer(
                f"<b>📃 Чек:</b> <code>#{get_purchase[4]}</code>\n"
                f"➖➖➖➖➖➖➖➖➖➖➖➖➖\n"
