...
async def turn_off_buy(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "<b>🔴 Покупки в боте были выключены</b>", reply_markup=get_settings_func()
    )
...
async def turn_on_buy(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "<b>🟢 Покупки в боте были включены</b>", reply_markup=get_settings_func()
    )
    await send_all_admin(
        f"👤 Администратор <a href='tg://user?id={message.from_user.id}'>{clear_firstname(message.from_user.first_name)}</a>\n"
...
@dp.message_handler(IsAdmin(), text="🔴 Отправить на технические работы", state="*")
...
@dp.message_handler(IsAdmin(), text="🟢 Вывести из технических работ", state="*")
...
async def get_text_for_change_faq(message: types.Message, state: FSMContext):
    ...
    try:
        await state.finish()
        await message.answer(
            f"ℹ FAQ был обновлён ✅ Пример:\n{msg}", reply_markup=get_settings_func()
        )
...
async def get_id_for_change_contact(message: types.Message, state: FSMContext):
    ...
    if msg.isdigit():
        ...
        if get_status_user is None:
            ...
            await state.finish()
            msg = (
                f"📕 <b>Писать сюда ➡ <a href='tg://user?id={msg}'>Администратор</a></b>"
            )
            update_settingsx(contact=msg)
            await message.answer(
                f"📕 Контакты были успешно обновлены ✅", reply_markup=get_settings_func()
            )
    else:
        await StorageSettings.here_contact.set()
        await message.answer(
